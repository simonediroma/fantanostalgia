...
**Branch attivo:** `claude/task-28-pattern-esplicativi-followup`
**PR in corso:** nessuna — branch pushato, PR non ancora aperta (follow-up al task 28, in attesa di richiesta esplicita). Nota: task 25 (PR #80), task 26 (PR #81), task 27 (PR #82) e task 28 (PR #83) sono stati mergiati da Simone direttamente; i follow-up per 25 (PR #84), 26 (PR #85) e 27 (PR #86) sono stati mergiati.
...
Task 24, 25, 26, 27 e 28 completati, più i follow-up di allineamento ai prompt aggiunti a posteriori per 25 (PR #84), 26 (PR #85), 27 (PR #86) e ora 28 (branch `claude/task-28-pattern-esplicativi-followup`, PR non ancora aperta — vedi voce **28-followup** in task list). Tutti e 3 i CSS applicativi sono migrati ai token/design-system condivisi, e `frontend/shared/design-system.js` è ora effettivamente caricato ed eseguito in produzione su admin + coach (Table/Badge/Button/ProgressBar/EmptyState/Modal/WizardSteps/DropZone/Panel/FileSpec/confirmDialog), non solo come libreria CSS. **Pattern confermato per l'epica: ogni volta che viene aggiunto un prompt a posteriori per un task già completato, va riletto e confrontato acceptance-criteria-vs-implementazione (è già successo per 25, 26, 27 e 28) — non dare per scontata la compliance solo perché il task risulta `[x]`.** Prossimo passo possibile: **task 29** (Gestione multi-lega in Admin, dipende da 26, sbloccato) oppure **task 31** (Pagina pubblica "Come Funziona", dipende da 25, sbloccato) — **task 30** resta HIGH risk (possibile modifica a `schema.sql`, richiede approvazione esplicita di Simone prima di procedere). **Attenzione:** come per 25-28, verificare a inizio sessione se esiste già un prompt in `prompts/frontend/29-multi-lega-admin.md` / `prompts/frontend/31-pagina-come-funziona.md`; se assente, chiarire lo scope con l'utente prima di procedere invece di assumere.
