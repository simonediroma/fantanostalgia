...
Task 24 e 25 completati (vedi task list sopra). Prossimo passo: **task 26 e 27** (parallelizzabili tra loro, dipendono solo da 24) — restyle Admin SPA e Coach SPA usando `frontend/shared/design-system.{js,css}`. **Attenzione:** a differenza del task 24, per 26/27/28/29/30/31 non esiste ancora un prompt scritto in `prompts/`; verificare all'inizio della sessione e, se assente, chiarire lo scope con l'utente prima di procedere (come fatto per il task 25) invece di assumere. Nota: `backend/static/style.css` è già stato migrato ai token condivisi nel task 25 (ora importa `frontend/shared/design-system.css` da `base.html`); restano da fare `frontend/coach/css/coach.css` e `frontend/admin/css/admin.css` nei task 26/27.
