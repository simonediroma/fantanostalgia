...
**PR in corso:** nessuna — branch pushato, PR non ancora aperta (task 27 completato, in attesa di richiesta esplicita). Nota: task 25 (branch `claude/prossimo-task-1wzl48`) è stato mergiato come PR #80; task 26 (branch `claude/prossimo-task-vwa0s9`) pushato, PR non ancora aperta.
...
Task 24, 25, 26 e 27 completati (vedi task list sopra). Tutti e 3 i CSS applicativi (`backend/static/style.css`, `frontend/admin/css/admin.css`, `frontend/coach/css/coach.css`) sono ora migrati ai token/design-system condivisi in `frontend/shared/`. Prossimo passo possibile: **task 28** (Pattern esplicativi help-box/file-spec/confirm-dialog, dipende da 26+27 — ora entrambi sbloccati) oppure **task 29** (Gestione multi-lega in Admin, dipende da 26). **Attenzione:** come per 25/26/27, verificare a inizio sessione se esiste già un prompt in `prompts/frontend/28-pattern-esplicativi.md` / `prompts/frontend/29-multi-lega-admin.md`; se assente, chiarire lo scope con l'utente prima di procedere invece di assumere (è successo per tutti i task 25-27 finora). Nota: i branch task 26 (`claude/prossimo-task-vwa0s9`) e 27 (`claude/prossimo-task-6xloxn`) sono pushati ma senza PR aperta — chiedere a Simone se vuole aprirle prima di proseguire con nuovi task, per evitare di accumulare troppi branch non mergiati.

Ordine di esecuzione: 24 (fatto) → 25 (fatto) → 26 (fatto) → 27 (fatto) → 28 (dopo 26+27, ora sbloccato) → 29 (dopo 26, sbloccato) → 30 (dopo 26+27, HIGH risk, sbloccato ma richiede approvazione su schema.sql) → 31 (dopo 25, sbloccato). Le task 32-35 restano bloccate finché non vengono risolte con `/cdp:spike-integrate` dopo aver riempito i rispettivi `docs/spike-NN.md`.
