...
**Branch attivo:** `claude/task-27-ds-factories-followup`
**PR in corso:** nessuna — branch pushato, PR non ancora aperta (follow-up al task 27, in attesa di richiesta esplicita). Nota: task 25 (PR #80), task 26 (PR #81), task 27 (PR #82) e task 28 (PR #83) sono stati mergiati da Simone direttamente; la #84 (follow-up task 25) e la #85 (follow-up task 26) sono state mergiate.
...
Task 24, 25, 26, 27 e 28 completati, più i follow-up di allineamento ai prompt aggiunti a posteriori per 25 (PR #84), 26 (PR #85) e ora 27 (branch `claude/task-27-ds-factories-followup`, PR non ancora aperta). Tutti e 3 i CSS applicativi sono migrati ai token/design-system condivisi, e `frontend/shared/design-system.js` è ora effettivamente caricato ed eseguito in produzione su admin + coach (Table/Badge/Button/ProgressBar/EmptyState/Modal/WizardSteps/DropZone/Panel), non solo come libreria CSS. **Se in futuro emergono prompt aggiunti a posteriori anche per gli altri task già completati (es. task 28), rifare lo stesso confronto acceptance-criteria-vs-implementazione prima di assumere che siano compliant.** Prossimo passo possibile: **task 29** (Gestione multi-lega in Admin, dipende da 26, sbloccato) oppure **task 31** (Pagina pubblica "Come Funziona", dipende da 25, sbloccato) — **task 30** resta HIGH risk (possibile modifica a `schema.sql`, richiede approvazione esplicita di Simone prima di procedere). **Attenzione:** come per 25-28, verificare a inizio sessione se esiste già un prompt in `prompts/frontend/29-multi-lega-admin.md` / `prompts/frontend/31-pagina-come-funziona.md`; se assente, chiarire lo scope con l'utente prima di procedere invece di assumere.
